...
from six import int2byte, BytesIO
...
class KeyTest (unittest.TestCase):
    ...
    def test_2_load_rsa(self):
        key = RSAKey.from_private_key_file(b'tests/test_rsa.key')
    ...
    def test_3_load_rsa_password(self):
        key = RSAKey.from_private_key_file(b'tests/test_rsa_password.key', b'television')
        self.assertEquals(b'ssh-rsa', key.get_name())
        my_rsa = hexlify(key.get_fingerprint())
        self.assertEquals(exp_rsa, my_rsa)
        self.assertEquals(PUB_RSA.split()[1], key.get_base64())
    ...
    def test_4_load_dss(self):
        key = DSSKey.from_private_key_file(b'tests/test_dss.key')
    ...
    def test_5_load_dss_password(self):
        key = DSSKey.from_private_key_file(b'tests/test_dss_password.key', b'television')
        self.assertEquals(b'ssh-dss', key.get_name())
        my_dss = hexlify(key.get_fingerprint())
        self.assertEquals(exp_dss, my_dss)
        self.assertEquals(PUB_DSS.split()[1], key.get_base64())
    ...
    def test_8_sign_rsa(self):
        ...
        sig = b''.join([int2byte(int(x, 16)) for x in SIGNED_RSA.split(b':')])
