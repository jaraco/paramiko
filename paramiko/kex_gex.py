...
class KexGex (object):
    ...
    def start_kex(self, _test_old_style=False):
        ...
        if _test_old_style:
            ...
            m.add_byte(int2byte(_MSG_KEXDH_GEX_REQUEST_OLD))
            m.add_int(self.preferred_bits)
            self.old_style = True
        else:
            m.add_byte(int2byte(_MSG_KEXDH_GEX_REQUEST))
    ...
    def _generate_x(self):
        ...
        while True:
            ...
            x_bytes = int2byte(ord(x_bytes[0]) & qmask) + x_bytes[1:]
    ...
    def _parse_kexdh_gex_request(self, m):
        ...
        m = Message()
        m.add_byte(int2byte(_MSG_KEXDH_GEX_GROUP))
        m.add_mpint(self.p)
        m.add_mpint(self.g)
        self.transport._send_message(m)
    ...
    def _parse_kexdh_gex_group(self, m):
        ...
        m = Message()
        m.add_byte(int2byte(_MSG_KEXDH_GEX_INIT))
        m.add_mpint(self.e)
        self.transport._send_message(m)
