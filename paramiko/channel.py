...
class Channel (object):
    ...
    def get_pty(self, term='vt100', width=80, height=24, width_pixels=0,
        ...
        m = Message()
        m.add_byte(int2byte(MSG_CHANNEL_REQUEST))
    ...
    def request_forward_agent(self, handler):
        ...
        m.add_byte(int2byte(MSG_CHANNEL_REQUEST))
        m.add_int(self.remote_chanid)
        m.add_string(b'auth-agent-req@openssh.com')
        m.add_boolean(False)
        self.transport._send_user_message(m)
    ...
    def recv(self, nbytes):
        ...
        if ack > 0:
            m = Message()
            m.add_byte(int2byte(MSG_CHANNEL_WINDOW_ADJUST))
            m.add_int(self.remote_chanid)
            m.add_int(ack)
            self.transport._send_user_message(m)
    ...
    def send(self, s):
        ...
        try:
            ...
            if size == 0:
                ...
                return 0
            m = Message()
            m.add_byte(int2byte(MSG_CHANNEL_DATA))
            m.add_int(self.remote_chanid)
            m.add_string(s[:size])
        finally:
            self.lock.release()
    ...
    def send_stderr(self, s):
        ...
        try:
            ...
            if size == 0:
                ...
                return 0
            m = Message()
            m.add_byte(int2byte(MSG_CHANNEL_EXTENDED_DATA))
            m.add_int(self.remote_chanid)
            m.add_int(1)
            m.add_string(s[:size])
        finally:
    ...
    def _handle_request(self, m):
        ...
        elif key == b'auth-agent-req@openssh.com':
            if server is None:
                ok = False
            else:
        ...
        if want_reply:
            m = Message()
            if ok:
                m.add_byte(int2byte(MSG_CHANNEL_SUCCESS))
            else:
                m.add_byte(int2byte(MSG_CHANNEL_FAILURE))
            m.add_int(self.remote_chanid)
            self.transport._send_user_message(m)
    ...
    def _send_eof(self):
        ...
        if self.eof_sent:
            return None
        m = Message()
        m.add_byte(int2byte(MSG_CHANNEL_EOF))
        m.add_int(self.remote_chanid)
        self.eof_sent = True
    ...
    def _close_internal(self):
        ...
        m1 = self._send_eof()
        m2 = Message()
        m2.add_byte(int2byte(MSG_CHANNEL_CLOSE))
